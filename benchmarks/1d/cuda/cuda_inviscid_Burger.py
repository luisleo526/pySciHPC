...
def cuda_inviscid_burger(f: Scalar, vel: Vector, solver: CudaDerivativesSolver, s: cp.ndarray, *args):
    neg_assign(solver.find_fx(burger_flux(f.data.gpu[0], solver.sol_buffer0), vel.x.cell.gpu[0]), s)
...
def run(N, source, bc, ghc, ts, scheme, dt):
    ...
    vel = Vector(**geo_dict)
    vel.x.cell.gpu[0] = phi.data.gpu[0]
