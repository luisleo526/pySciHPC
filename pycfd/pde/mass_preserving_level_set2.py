...
def mpls_source(f: np.ndarray, grids: np.ndarray, ghc: int32, ndim: int32, gradient: Callable, width: float64,
                imass: float64, density_ratio: float64, delta0: np.ndarray):
    dx, dy, dz = grids
    if ndim > 3:
        dv = dx * dy * dz
    else:
...
def mpls2(temproal: Callable, gradient: Callable, phi: np.ndarray, grids: np.ndarray, ghc: int32, ndim: int32,
